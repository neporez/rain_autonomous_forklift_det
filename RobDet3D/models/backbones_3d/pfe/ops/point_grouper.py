...
@grouper.register_module('neighbour_idx_group')
class NeighborIdxGrouping(PointGrouping):
    def __init__(self, grouping_cfg, input_channels, *args, **kwargs):
        super(NeighborIdxGrouping, self).__init__(grouping_cfg, input_channels)

        radius = self.cfg.query.radius
        self.radius = [radius] if isinstance(radius, float) else radius
        self.neightbour = self.cfg.query.neighbour
        querier_type = 'neighbour'
        self.querier = partial(querier.from_name(querier_type), *self.radius, self.neightbour)


    def query_and_group(self, new_xyz: torch.Tensor, xyz: torch.Tensor, feats: torch.Tensor = None, sampled_ind: torch.Tensor = None):

        group_member_cnt, group_member_ind = self.querier(xyz, new_xyz, sampled_ind)
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor, feats: torch.Tensor = None, sampled_ind: torch.Tensor = None , *args, **kwargs):
        """
             ...
             :return:
                 new_feats: (B, M, Co)
        """
        empty_mask, new_feats = self.query_and_group(new_xyz, xyz, feats, sampled_ind)
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor, feats: torch.Tensor = None, *args, **kwargs):
        """
             ...
             :return:
                 new_feats: (B, M, Co)
        """
        feat_feats = apply1d(self.feat_encoder, feats)  # (B,N,C1)
        

        attention = apply1d(self.attention_encoder, feats)  # (B,N,C1)
