...
from .ops.builder import sampler, grouper, aggregation, serializer
...
class GeneralPointSetAbstraction(nn.Module):
    ...
    def __init__(self, ab_layer_cfg, input_channels: int):
        ...
        self.serializer = nn.ModuleList()
        for serializer_cfg in self.cfg.get('serializer', []):
            self.serializer.append(serializer.from_cfg(serializer_cfg))
    ...
    def forward(self,
        ...
        Returns:
            ...
            new_feats: (B, M, Co)
        """
                # Optional serialization step
        if len(self.serializer) > 0:
            # assume each serializer returns sorted index: (B, N)
            for s in self.serializer:
                sorted_idx = s(xyz=xyz, bid=bid)  # (B,N)
                # Apply sort to xyz, feats, bid
                xyz = gather(xyz, sorted_idx).contiguous()
                if feats is not None:
                    feats = gather(feats, sorted_idx).contiguous()            
        ...
        group_list = [g(xyz, new_xyz, feats, sample_ind, new_feats, bid, new_bid) for g in self.groupers]
        new_feats = self.agg(*[list(ret_i) for ret_i in zip(*group_list)])
        return new_xyz, new_feats, new_bid
    
    def forward_v2(self,
                xyz: torch.Tensor, feats: torch.Tensor = None, bid=None, sample_ind: torch.Tensor = None,
                new_xyz=None, new_feats=None, new_bid=None) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        """
        Args:
            xyz: (B, N, 3)
...
class PointVoteInsCenter(nn.Module):
    ...
    def forward(self,
                start_xyz: torch.Tensor, start_feats: torch.Tensor, start_bid: torch.Tensor, vote_ind: torch.Tensor,
        ...
        _, vote_end_feats, _ = self.sa_layers.forward_v2(
            new_xyz=vote_end,
