...
void print_cnt_distribution(const uint32_t *cntPtr, int B, int M, int samplePerBall, float radius) {
    std::vector<uint32_t> hostCnt(B * M);

    // GPU → CPU 복사
    cudaMemcpy(hostCnt.data(), cntPtr, sizeof(uint32_t) * B * M, cudaMemcpyDeviceToHost);

    // 정렬
    std::sort(hostCnt.begin(), hostCnt.end());

    auto percentile = [&](float p) {
        size_t idx = static_cast<size_t>(p * (hostCnt.size() - 1));
        return hostCnt[idx];
    };

    uint32_t min = hostCnt.front();
    uint32_t max = hostCnt.back();
    uint32_t p25 = percentile(0.25f);
    uint32_t p50 = percentile(0.50f);
    uint32_t p75 = percentile(0.75f);

    // ✅ samplePerBall 초과 개수 및 비율 계산
    size_t overflow_count = std::count_if(hostCnt.begin(), hostCnt.end(), [&](uint32_t x) {
        return x >= samplePerBall;
    });
    float overflow_percent = 100.0f * overflow_count / hostCnt.size();

    std::cout << "[GridBallQuery] mSamplePerBall: " << samplePerBall
              << ", mRadius: " << radius << "\n";
    std::cout << "[GridBallQuery] Point Count Percentiles over " << (B * M) << " queries:\n";
    std::cout << "  Min (0%)  : " << min << "\n";
    std::cout << "  25%       : " << p25 << "\n";
    std::cout << "  50% (Med) : " << p50 << "\n";
    std::cout << "  75%       : " << p75 << "\n";
    std::cout << "  Max (100%): " << max << "\n";
    std::cout << "  Overflow  : " << overflow_count << " queries ("
              << std::fixed << std::setprecision(2) << overflow_percent << "% > samplePerBall)\n";

    // 🔽 마지막 일부 값 출력
    std::cout << "[GridBallQuery] cntPtr last 300 values of each batch:\n";
    for (int b = 0; b < B; ++b) {
        std::cout << "Batch " << b << ": ";
        for (int m = M-300; m < M; ++m) {
            std::cout << hostCnt[b * M + m] << " ";
        }
        std::cout << "\n";
    }
}
...
int32_t GridBallQueryPlugin::enqueue(nvinfer1::PluginTensorDesc const *inputDesc,
    ...
    try {
        ...
        // print_cnt_distribution(cntPtr, batchSize, querySize, mSamplePerBall, mRadius);
