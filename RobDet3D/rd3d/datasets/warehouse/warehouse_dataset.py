...
from .warehouse_toolkits import Octopus


@datasets.register_module()
class WarehouseDataset(DatasetTemplate):
    CLASS_NAMES = ('Pedestrian')
    
    def __init__(self, dataset_cfg, class_names=CLASS_NAMES, training=True, root_path=None, logger=None):
        """
        Args:
            root_path:
            dataset_cfg:
    ...
    def __getitem__(self, index):
        ...
        input_dict = {
            ...
            'epoch_current': getattr(self, 'epoch_cuurent',0)
        }
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('warehouse_dbinfos_%s.pkl' % split)
...
def create_once_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = WarehouseDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    ...
    for split in splits:
        ...
        filename = 'warehouse_infos_%s.pkl' % split
        filename = save_path / Path(filename)
        dataset.set_split(split)
        warehouse_infos = dataset.get_infos(num_workers=workers)
        with open(filename, 'wb') as f:
            pickle.dump(warehouse_infos, f)
        print('Warehouse info %s file is saved to %s' % (split, filename))

    train_filename = save_path / 'warehouse_infos_train.pkl'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--func', type=str, default='create_warehouse_infos', help='')
    args = parser.parse_args()

    if args.func == 'create_warehouse_infos':
        
        spec = importlib.util.spec_from_file_location("cfg", args.cfg_file)
        cfg_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(cfg_module)
        dataset_cfg = cfg_module.DATASET


        ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        warehouse_data_path = ROOT_DIR / 'data' / 'warehouse'
        warehouse_save_path = ROOT_DIR / 'data' / 'warehouse'

        create_once_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Pedestrian'],
            data_path=warehouse_data_path,
            save_path=warehouse_save_path
        )