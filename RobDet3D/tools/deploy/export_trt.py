...
def build_engine():
    ...
    profile = set_profile(builder.create_optimization_profile(), onnx_model, (1, 1, 1))
    ...
    if hasattr(args, "dla") and args.dla:
        config.default_device_type = trt.DeviceType.DLA     # ✅ 더 안전한 설정
        config.DLA_core = args.dla_core
        config.set_flag(trt.BuilderFlag.GPU_FALLBACK)


    output_path = fold / (file + "_" + args.type + '.engine')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dla', action='store_true', help='use DLA')
    parser.add_argument('--dla_core', type=int, default=0, help='select DLA core id')
