...
def add_args():
    ...
    parser.add_argument('--scenes', type=int, default=5, help='random seed')
    return parser
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for ind in scenes:
            ...
            batch_dict['gt_boxes'] = batch_dict['gt_boxes'].view(-1, 8)

            labels = pred_dicts[0]['pred_labels'].float().unsqueeze(1) 

            pred_dicts[0]['pred_boxes'] = torch.cat([pred_dicts[0]['pred_boxes'].view(-1, 7), labels], dim=1)       

            pred_dicts[0]['pred_boxes'] = pred_dicts[0]['pred_boxes'][(pred_dicts[0]['pred_scores'] > 0.1),:]

            print(pred_dicts[0]['pred_scores'])
            print(pred_dicts[0]['pred_labels'])

            print(batch_dict['gt_boxes'][0],  pred_dicts[0]['pred_boxes'][0])


            viz_scenes((batch_dict['points'].view(-1, 5), batch_dict['gt_boxes']),
                       (batch_dict['points'].view(-1, 5), pred_dicts[0]['pred_boxes']),
                       offset=(0, 100, 0))
    logger.info("Done")
